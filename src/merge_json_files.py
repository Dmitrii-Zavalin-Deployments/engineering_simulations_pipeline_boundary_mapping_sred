...
def merge_json_files(mesh_file, initial_file, output_file):
    ...
    # Load mesh data (now correctly treating it as a top-level structure)
    ...
    final_data = {
        "mesh": mesh_data,  # Uses the entire first JSON file as the mesh structure
        "boundary_conditions": fluid_data.get("boundary_conditions", {}),  # Fluid conditions
        "fluid_properties": fluid_data.get("fluid_properties", {}),  # Fluid properties
        "simulation_parameters": fluid_data.get("simulation_parameters", {})  # Simulation settings
    }
